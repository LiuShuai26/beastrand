# projects/pong/config.py
"""Configuration for Pong self-play training.

Mirrors projects/tennis/config.py (same Atari pipeline, same self-play
mechanics) but targets the PettingZoo pong_v3 env. Used to validate the
self-play framework on a task with a clean single-agent baseline
(PongNoFrameskip-v4 PPO reaches ~+20 in 10M steps).
"""
...
class Args:
    ...
    make_env_path: str = field(default="projects.pong.make_env.make_env", init=False)

    bootstrap_value: bool = field(default=True, init=False)

    # --- Multi-agent / Self-play ---
    num_agents: int = field(default=2, metadata={"help": "Agents per env (2 for Pong)"})
    ...
    bot_ratio: float = field(
        default=0.0,
        metadata={"help": "Probability per episode of playing vs ALE scripted bot (single-agent PongNoFrameskip-v4). Applied before self_play_mode dispatch — when not bot, fall through to existing snapshot/latest/mixed sampling."},
    )
    ...
    # --- Environment ---
    env_id: str = field(default="pong_v3", metadata={"help": "Env identifier (used for run naming)"})
    ...
    # Pong uses AtariPolicy on pixel images. See projects/atari/config.py for
    ...
    def make_run_name(self) -> str:
        ...
        return f"pong_selfplay_{int(time.time())}"
    ...
    def validate(self) -> None:
        ...
        assert self.num_agents == 2, "Pong is a 2-player game"
        ...
        if not (0.0 <= self.bot_ratio <= 1.0):
            raise ValueError("bot_ratio must be in [0, 1]")
